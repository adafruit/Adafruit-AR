...
class FocusSquare: SCNNode {
  ...
  private let focusSquareSize: Float = 0.3
  
  // Thickness of the focus square lines in m.
  private let focusSquareThickness: Float = 0.09
  ...
  private let focusSquareColor = #colorLiteral(red: 0.9529411793, green: 0.6862745285, blue: 0.1333333403, alpha: 1) // base yellow
  private let focusSquareColorLight = #colorLiteral(red: 1, green: 0.9312674403, blue: 0.4846551418, alpha: 1) // light yellow
  
  
  
  public var placementMessage = String()
  ...
  func update(for position: SCNVector3, planeAnchor: ARPlaneAnchor?, camera: ARCamera?) {
    ...
    if let anchor = planeAnchor {
      ...
      placementMessage = "TAP TO PLACE ADABOT"
    }
     else {
      placementMessage = "MOVE AROUND TO SCAN A SURFACE"
      open()
    }
